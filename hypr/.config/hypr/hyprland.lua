...
local fileManager = terminal .. "bash --rcfile " .. hyprScripts .. "/dired"
local menu = "fuzzel"
local editor = terminal .. "bash --rcfile "  .. hyprScripts .. "/editor"
local llmagent = terminal .. "opencode"
...
hl.on("hyprland.start", function()
          ...
          hl.exec_cmd("systemctl --user import-environment GTK_THEME XCURSOR_THEME XCURSOR_SIZE HYPRCURSOR_THEME HYPRCURSOR_SIZE GTK_CURSOR_THEME QT_QPA_PLATFORMTHEME")
          hl.exec_cmd("waybar")
...
hl.bind(mainMod .. " + R", hl.dsp.exec_cmd(menu .. " || pkill " .. menu))
hl.bind(mainMod .. " + Q", hl.dsp.exec_cmd("freetube"))
...
-- Switch workspaces with mainMod + numbers on AZERTY
...
hl.bind(mainMod .. " + egrave", hl.dsp.focus({ workspace = 6}))
hl.bind(mainMod .. " + minus", hl.dsp.focus({ workspace = 7}))
...
hl.config({
        general = {
            ...
            col = {
                active_border   = { colors = {season_bright, season_dim}, angle = 45 },
                inactive_border = season_dim,
            },
...
hl.curve("snappy",         { type = "spring", mass = 1, stiffness = 400,    dampening = 34 })
...
hl.window_rule({
        name = "keepass-unlock-float",
        match = { title  = "Unlock Database - KeePassXC"},
        float = true,
})

hl.window_rule({
        name = "keepass-generator-float",
        match = { title  = "Generate Password"},
        float = true,
})

hl.window_rule({
        name = "keepass-permission-float",
        match = { title  = "KeePassXC -  Access Request"},
        float = true,
})

hl.window_rule({
        name = "keepass-create-db",
        match = { title  = "Create a new KeePassXC database..."},
        float = true,
})
